...
codebooks                    = ["codebook_string_488.csv"] #name of codebooks
exp_dir                      = "250310_150_nih3t3_current" #name of experimental directory
user                         = "Lex" #name of user
num_rounds                   = 4     #number of rounds
first_radius                 = 1.5   #first search radii
second_radius                = 1.5   #second search radii
...
total_hybs                   = 24    #number of total hybs
probability_cutoff           = 0.10  #probability cutoff for On dots (0-1). Lower the value the less stringent.
desired_fdr                  = 0.06  #desired FDR (0-1). Could set to None if you would like to filter yourself.
...
#path to dots
locations_path = glob.glob(f"/groups/CaiLab/personal/{user}/raw/{exp_dir}/pyfish_tools/output/dots_detected/Channel_{channel}/spots_in_cells/Pos{JOB_ID}/locations*")
#____________________________________________________________________________________________________________________________
...
if parity_round:
    for diff in range(2):
        ...
        output_dir = f"/groups/CaiLab/personal/{user}/raw/{exp_dir}/pyfish_tools/output/correct_decoded_scaled_noiseinc_normscore/final_{first_radius}{second_radius}{third_radius}_seed{min_seed}{high_exp_seed}_heg_svm_p{probability_cutoff*100}_diff{diff}_fdr{desired_fdr*100}/Channel_{channel}/Pos_{JOB_ID}"
    ...
    #Where do you want to output the files
    output_dir = f"/groups/CaiLab/personal/{user}/raw/{exp_dir}/pyfish_tools/output/decoded_scaled/final_{first_radius}{second_radius}{third_radius}_seed{min_seed}{high_exp_seed}_heg_svm_p{probability_cutoff*100}_diff{diff}_fdr{desired_fdr*100}/Channel_{channel}/Pos_{JOB_ID}"
    if len(locations_path) > 1:
        for locations in locations_path:
            try:
                decoder = decode(location_path=locations, codebook_path=codebook_path,
