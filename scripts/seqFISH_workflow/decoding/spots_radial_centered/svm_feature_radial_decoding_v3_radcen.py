...
class decode:
    ...
    def poly_rbf_gen_dot_probabilities(dots_used_trues, dots_used_fakes, locations):
        ...
        """
        #get attributes
        trues = dots_used_trues[["sigma", "peak intensity", "sharpness_l", "sharpness_t", "symmetry"]]
        trues["labels"] = 1
        fakes = dots_used_fakes[["sigma", "peak intensity", "sharpness_l", "sharpness_t", "symmetry"]]
        fakes["labels"] = -1
        ...
        X_train, X_test, y_train, y_test = train_test_split(comb.iloc[:,0:5], 
        ...
        locations_features = locations[["sigma", "peak intensity", "sharpness_l", "sharpness_t", "symmetry"]]
        #normalize data
            ...
            for i in range(len(neighbor_list2)):
                ...
                for j in range(len(neighbor_list2[i])):
                    if len(neighbor_list2[i][j]) == 1:
                        ...
                        #rank dots 
                        int_score = np.argsort(dot_traits["peak intensity"]).values[::-1]
                        size_score = np.argsort(dot_traits["sigma"]).values[::-1]
    ...
    def radial_decoding_parallel(self, locations, codebook, num_barcodes = 4, radius=1, diff=0,
        ...
        for i in range(len(dot_info)):
            ...
            if "cell number" in dot_info[i]:
                info = dot_info[i][["x", "y", "z", "ch", "sigma", "peak intensity", "sharpness_l", "sharpness_t", "symmetry", "cell number"]].mean().values
            else:
                info = dot_info[i][["x", "y", "z", "ch", "sigma", "peak intensity", "sharpness_l", "sharpness_t", "symmetry"]].mean().values 
        ...
        if diff == 0:
            ...
            if len(genes_locations.columns) == 10:
                genes_locations.columns = ["x", "y", "z", "ch", "sigma", "peak intensity", "sharpness_l", "sharpness_t", "symmetry", "cell number"]
            else:
                genes_locations.columns = ["x", "y", "z", "ch", "sigma", "peak intensity", "sharpness_l", "sharpness_t", "symmetry"]
            ...
            if "cell number" in genes_locations:
                genes_locations = genes_locations[["genes","x", "y", "z", "ch", "sigma", "peak intensity", "sharpness_l", "sharpness_t", "symmetry", "ambiguity score", "codeword score", "cell number"]] 
            else:
                genes_locations = genes_locations[["genes", "x", "y", "z", "ch", "sigma", "peak intensity", "sharpness_l", "sharpness_t", "symmetry", "ambiguity score", "codeword score"]] 
