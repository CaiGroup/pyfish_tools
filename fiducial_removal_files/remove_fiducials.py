...
def remove_all_fiducials(locations_src, fid_src, threshold=500, radius=1, swapaxes=True, write =True):
    """
    ...
    radius: pixel search
    swapaxes: bool to swap axes (fiducial image needs to be z,c,x,y)
...
def remove_fiducials_parallel(locations_srcs, fid_srcs, threshold=500, radius=1, swapaxes=True):
    """
    ...
    if len(locations_srcs) == 1:
        remove_all_fiducials(locations_src, fid_srcs, threshold=threshold, radius=radius, swapaxes=swapaxes, write =True)
        ...
        with ProcessPoolExecutor(max_workers=20) as exe:
            for i in range(len(locations_srcs)):
                exe.submit(remove_all_fiducials, locations_srcs[i], fid_srcs[i],
                           threshold=threshold, radius=radius, swapaxes=swapaxes, write =True )
