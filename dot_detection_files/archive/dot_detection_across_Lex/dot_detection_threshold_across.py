...
def dot_counts(img_src, min_sigma = 1.5, max_sigma = 5, num_sigma = 5, threshold = 0.001, channel=0):
    ...
    if channel == "all":
        ...
        if len(img.shape)== 3:
            for c in range(img.shape[0]):
                blobs_log = blob_log(img[c], min_sigma = min_sigma, max_sigma=max_sigma, 
            ...
            for c in range(img.shape[1]):
                for z in range(img.shape[0]):
                    blobs_log = blob_log(img[z][c], min_sigma = min_sigma, max_sigma=max_sigma, 
                                         num_sigma=num_sigma, threshold=threshold)
                    counts += len(blobs_log)
                #make list
                _list = [threshold, counts, c]
    ...
    #blob detection across z and one channel
    else:
        ...
        if len(img.shape)== 3:
            ...
            _list = [threshold, counts, channel]
            #make textfile
...
def find_threshold(img_src, min_sigma = 1.5, max_sigma = 5, 
    ...
    channel = which channel to look at or "all channels"
    min_dots_start= minimum number of dots to start iteration
    min_dots_end= minimum number of dots to end iteration
    num_pos = positions used for finding thresholds
    strict = if true uses min_dots as upper bound instead of lower
