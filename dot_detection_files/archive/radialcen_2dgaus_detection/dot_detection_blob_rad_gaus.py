...
def sliding_lineregress(arr, window=10, reduce_cutoff = 2):
    ...
    #go through and data points and fit line
...
def find_threshold(img_src, min_sigma = 1.5, max_sigma = 5, 
                   ...
                   min_dots_end = 11000,pos_start=5,pos_end=10, 
                   reduce_cutoff= 2, window=10, strict=False):
    ...
    strict = if true it will treat min dots as the upper bound
...
def dot_detection(img_src, min_sigma = 1.5, max_sigma = 5, 
    ...
    if optimize == True:
        ...
        if (gaussian == False) and (radial_center == False):
            ...
            rad_df["size"] = area_list
            rad_df["hyb"] = HybCycle
            rad_df = rad_df[["hyb","ch","x","y","z","intensity", "sigma"]]
            #write out plot
            output_path_adj = output_path.replace(".csv", f"_{opt_thresh}.csv")
            #filter by size
            mu, std = norm.fit(rad_df["size"]) #fit gaussian to size dataset
...
def dot_detection_parallel(img_src, min_sigma = 1.5, max_sigma = 5, 
    ...
    if (optimize == True) and (type(img_src) != list):
            ...
            if radial_center == False:
                ...
                pos = None
                for opt_thresh in thresh_median:
                    dot_detection(img_src, min_sigma, max_sigma, 
                                  num_sigma, HybCycle, size_cutoff,
                                  opt_thresh,channel,pos,choose_thresh_set,hyb_number,region_size,
                                  check_initial, gaussian, radial_center, optimize, output)
                    print(f'Threshold{opt_thresh} completed after {(time.time() - start)/60} minutes')
        ...
        if radial_center == False:
            ...
            for img in img_src:
                ...
                HybCycle_mod = int(img_parent_cycle.split("_")[1])
                #dot detect
                dot_detection(img, min_sigma, max_sigma, 
                              num_sigma, HybCycle_mod, size_cutoff,
                              opt_thresh,channel,pos,choose_thresh_set,hyb_number,region_size,
                              check_initial, gaussian,radial_center, optimize, output)