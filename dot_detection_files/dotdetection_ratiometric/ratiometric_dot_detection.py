...
def ratiometric_dot_detection(img_src, HybCycle=0, size_cutoff=None,
                  threshold=500, radius = 2, swapaxes=False):
    """
    ...
    swapaxes: bool to flip z and c axis (image needs to be z,c,x,y)
    """
...
def pixel_based_detection(img_src, HybCycle=0,threshold=500, 
                          size_cutoff=None, swapaxes=False):
...
def ratiometric_dot_detection_parallel(img_src, size_cutoff=None, threshold=500,
                                       radius = 1.5, pixel_based = False, swapaxes=False):
    ...
    if pixel_based == False:
        output_folder = parent / "notebook_pyfiles"/ "dots_detected" / "spot_based"
    else:
        output_folder = parent / "notebook_pyfiles"/ "dots_detected" / "pixel_based"
