...
def dot_counts(img_src, threshold = 200, box_size = 5, channel=0, exclude_border = 3):
    """
    ...
    counts = 0
    if len(img.shape)== 3:
        peaks = find_peaks(img[channel], threshold=threshold, box_size=box_size, border_width=exclude_border)
        counts += len(peaks)
        _list = [threshold, counts]
    else: 
        for z in range(img.shape[0]):
            peaks = find_peaks(img[z][channel], threshold=threshold, box_size=box_size, border_width=exclude_border)
            counts += len(peaks)
        _list = [threshold, counts]
...
def find_threshold(img_src, box_size = 5,exclude_border=3, threshold_min = 100, 
                   ...
                   HybCycle = 0, channel = 0,pos_start=5,pos_end=10, 
...
def dot_detection(img_src, box_size = 5, exclude_border=3, HybCycle=0, size_cutoff=3, 
                  opt_thresh=0.001,channel=0,pos=0,choose_thresh_set = 0, hyb_number=64,
    ...
    if optimize == True:
        ...
        if len(img.shape)==3:
            z=0
            peaks = find_peaks(img[channel], threshold=opt_thresh, box_size=box_size, border_width=exclude_border)
            ...
            dots = []
            for z in range(img.shape[0]):
                peaks = find_peaks(img[z][channel], threshold=opt_thresh, box_size=box_size, border_width=exclude_border)
                peaks = peaks.to_pandas()
                peaks = peaks[["x_peak" ,"y_peak"]].values
                ch = np.zeros(len(peaks))+channel+1
                z_slice = np.zeros(len(peaks))+z
