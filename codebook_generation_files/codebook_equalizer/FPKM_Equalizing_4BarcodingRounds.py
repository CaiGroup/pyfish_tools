...
def AssignHighExpressionCodewords(FPKM_sorted, pseudocolors):
    ...
    for i in range(0,pseudocolors):
        ...
        hyb4 = Range[(int(hyb1 + hyb2 + hyb3)%pseudocolors-1)]

        HighExpressionCodewords.append([hyb1, hyb2, hyb3, hyb4])

    HighExpressionCodewords


    Codewords = pd.DataFrame(HighExpressionCodewords, columns = ['hyb1', 'hyb2', 'hyb3', 'hyb4'])
    Codewords
...
def FPKMSforCodebook(Codebook, FPKMS_table, numColors):
    ...
    counts_per_hyb= pd.DataFrame(0., index = np.arange(numColors+1) ,columns = ['hyb1', 'hyb2,', 'hyb3', 'hyb4'])
    counts_per_hyb.columns = ['hyb1', 'hyb2', 'hyb3', 'hyb4']
...
def FindNextCodeWord(FPKMS, pseudocolors):
    ...
    ParityHyb = OrderOfLeast.index(4) + 1
    #print(ParityHyb)
    ...
    if (ParityHyb == 1):
        Hyb1 = int(InverseModulo(Hyb2, Hyb3, Hyb4, pseudocolors))
        #print('Hyb1', Hyb1)

    if (ParityHyb == 2):
        Hyb2 = int(InverseModulo(Hyb1, Hyb3, Hyb4, pseudocolors))
        #print('Hyb2', Hyb2)

    if (ParityHyb == 3):
        Hyb3 = int(InverseModulo(Hyb1, Hyb2, Hyb4, pseudocolors))
        #print('Hyb3', Hyb3)

    if (ParityHyb == 4):
        Hyb4 = int(Range[int(Hyb1 + Hyb2 + Hyb3)%pseudocolors-1])
        #print('Hyb4', Hyb4)
    
    return [Hyb1, Hyb2, Hyb3, Hyb4]


def InverseModulo(FirstHyb, SecondHyb, ParityValue, pseudocolors):
    '''
    An inverse modulus function to caluculate a barcoding round pseudocolor assuming Parity Check, where (hyb1 + hyb2 + hyb3)%pseudcolors = hyb5.
    ...
    for i in range(0,pseudocolors): 
        if PossibleValues[(FirstHyb + SecondHyb + i)%pseudocolors] == ParityValue:
...
def CreateFullCodebook(UsedCodebook, EntireCodebook, FPKMS_table, numColors):
    ...
    while len(UsedCodebook)<len(FPKMS_table):
        ...
        while (i<50): 
            if (CheckIfCodeIsPresent(NextCode, RemainingCodebook)):
                ...
                RemainingCodebook = RemainingCodebook.drop(RemainingCodebook[(RemainingCodebook['hyb1'] == ArrayToAppend[0])
                    ...
                    & (RemainingCodebook['hyb4'] == ArrayToAppend[3])].index)
        ...
        if (i==50): 
            ...
            ArrayToAppend = list(TempDfToFindCodeword[1:5]) + [NextGene]
            print(ArrayToAppend)
            TempDfToAppend = pd.DataFrame(columns = ['hyb1', 'hyb2', 'hyb3', 'hyb4', 'Gene'])
    ...
    UsedCodebook = UsedCodebook.astype({"hyb1":'int', "hyb2":'int', "hyb3":'int', "hyb4":'int'})     
    ...
    for q in Range:
        for w in Range: 
            for e in Range: 
                ...
                if (t == 0):
                    t = len(Range)
                Codebook.append([q,w,e,t])
                
    assert(len(Codebook) == pseudocolors**3)
...
def CreateEmptyCodebook(listOfBarcodes):
    ...
    listOfBarcodes - An array of barcode arrays. E.g. [[1,2,3,4], [3,4,5,6]]
    ...
    maindict.columns = ['hyb1', 'hyb2', 'hyb3', 'hyb4']
...
def GenerateCodebook(pc, FPKMS):
    '''
    User friendly final function to generate Barcodes for each gene. Generates the barcodes such that they satisfy the parity check ('hyb1' + 'hyb2' + 'hyb3')%pseudocolors = 'hyb4'. Any barcode that satisfies this parity check may be assigned to a gene. 
...
def VerifyCodebook(Codebook):
    ...
    CopyFullCodebook = CopyFullCodebook.drop_duplicates(subset = ['hyb1', 'hyb2', 'hyb3', 'hyb4'], keep = 'first')
