...
def pick_best_codeword(filtered_set, codeword_scores,total_distances):
    ...
    for codes in filter_list:
        ...
        if len(np.where(track_score == np.amax(track_score))[0]) == 2:
            best_code = np.argmin(track_distance)
            final_codewords.append(list(codes[best_code]))
        else:
            best_code = np.argmax(track_score)
            final_codewords.append(list(codes[best_code]))
...
#     #remove any dot index repetitions
#     #convert lists to sets
#     sets = set({frozenset(l) for l in dot_idx})
#     #find repeating values
#     values, _, counts = np.unique(np.concatenate(dot_idx), return_counts=True, return_index=True)
#     repeats = set(np.compress(counts>1, values))
